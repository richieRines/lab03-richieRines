...
935 observations and 26 varaibles. Each row represents a different laureate.
...
The 4 bar graphs show that the majority of winners are from the US, so yes, the Buzzfeed headline is correct.
...
```{r}
nobel_living <- nobel_living %>%
  mutate(born_country_us = if_else(born_country_us == "USA", "USA", "Other"))
```
...
nobel_living <- nobel_living %>%
  mutate(country_us = if_else(country == "USA", "USA", "Other"),
         born_country_us = if_else(born_country_us == "USA", "USA", "Other"))
...
ggplot(nobel_living_science, aes(y = country_us, fill = born_country_us)) +
  geom_bar() +
  facet_wrap(. ~ category) +
  labs(x = "Count", y = "Where they were when they won", fill = "Birth country")
```

Buzzfeed's claim appears to be accurate. Looking at the graphs, we can see that a decent chunk (although not the majority) of USA winners were born in other coutnries. In the chemistry category, for example, 20 USA winners were born in the US, while 12 of them were born in other countries.
